...
def plot_convection_circles(DF, idx, fig=None, base_color=None, ax=None, age_plot=False, age_scale=10**-9, age_label='Gyr'):
    ...
    final_mass = DF.iloc[-1]['star_mass']
    ...
    ax.set_xlim(-1.2 * final_mass, 1.2 * final_mass)
    ax.set_ylim(-1.2 * final_mass, 1.2 * final_mass)
    ax.set_xticks([])
    ax.set_yticks([])



def create_image_folder(DF, folder_name='./images', start=0, end=None, age_plot=False):
    if end is None:
...
def create_video(image_folder, video_name='starmovie.avi', fps=15, scale_by_age=False, DF=None):
    ...
    if scale_by_age:
        min_timestep = min([sorted(DF['star_age']).iloc[i+1] - sorted(DF['star_age']).iloc[i] for i in range(len(DF)) if i+1  < len(DF)])
...
def movie(history_file, image_folder_name='./images', video_name='starmovie.avi', age_plot=False, fps=15, end=None, scale_by_age=False):
    DF = pd.read_table(history_file, skiprows=5, sep=r'\s+')

    print(f'creating folder of images: {image_folder_name}')
    create_image_folder(DF, folder_name=image_folder_name, age_plot=age_plot, end=end)

    print(f'creating video {video_name}')
    create_video(image_folder_name, video_name=video_name, fps=fps, scale_by_age=scale_by_age, DF=DF)
